# Building Your First AI Agent: From Code to Cloud
...
#### Configure AWS Credentials (Notepad)
...
| Docstring | Tells the agent what the tool does (important!) |
| Type hints | Help the agent understand what parameters to pass |
| `tools=[...]` | Registers tools with the agent |
...
### Step 4.1: Create agentcore_integration.py

Create a new file called `agentcore_integration.py`:

```python
# ============================================================
# Phase 3: Deploy to AWS with Amazon Bedrock AgentCore
# ============================================================
...
# ============================================================
# AGENT: Lazy initialization to avoid timeout during cold start
# ============================================================
...
> **Why is the agent wrapped in `get_agent()` here when it wasn't in `simple_agent.py`?**
>
> `simple_agent.py` and `agent_with_tools.py` run on your laptop, where you can wait as long as you want for the agent to build. This file runs inside an **AWS AgentCore container** that has a strict **30-second cold-start window** — if the container isn't ready in time, AgentCore marks it as failed.
>
...
### The Connection: Local Code → Cloud Runtime

Think of it this way:

```
┌─────────────────────────────────────────────────────────────┐
│  YOUR CODE (agentcore_integration.py)                       │
│                                                             │
│  @app.entrypoint                                            │
│  def invoke(payload):        <── AgentCore calls THIS       │
│      response = get_agent()(payload["prompt"])              │
│      return str(response)                                   │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
...
│  • Calls your invoke() function                             │
│  • Returns the response                                     │
│  • Handles scaling, security, logging                       │
└─────────────────────────────────────────────────────────────┘
```
...
```cmd
"C:\Users\YOUR_USERNAME\AppData\Roaming\Python\Python313\Scripts\agentcore.exe" configure --entrypoint agentcore_integration.py --name wanderly_travel_agent --deployment-type container --disable-memory --non-interactive
```

**Mac/Linux:**
```bash
agentcore configure --entrypoint agentcore_integration.py --name wanderly_travel_agent --deployment-type container --disable-memory --non-interactive
```

**What this does:**
- `--entrypoint agentcore_integration.py` — Points to your code file
